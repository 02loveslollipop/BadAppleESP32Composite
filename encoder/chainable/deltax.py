"""
Delta encoding component for video frame compression.

This component provides efficient frame delta encoding with optional RLE compression,
...
try:
    from ..cpu.delta_encode import compute_frame_deltas_cpu, compress_deltas_rle_cpu
    CPU_AVAILABLE = True
except ImportError:
    CPU_AVAILABLE = False
    compute_frame_deltas_cpu = None
    compress_deltas_rle_cpu = None

try:
    import cupy as cp
    from ..cuda.delta_encode import compute_frame_deltas_gpu, compress_deltas_rle_gpu
    CUDA_AVAILABLE = True
except ImportError:
    CUDA_AVAILABLE = False
    compute_frame_deltas_gpu = None
    compress_deltas_rle_gpu = None


class VideoDeltaEncoder(ChainComponent):
    """Component for delta encoding video frames with optional RLE compression."""
    ...
    def __init__(self,
                 ...
                 workers: int = 4,
                 batch_size: int = 50):
        super().__init__("VideoDeltaEncoder")
    ...
    def _validate_input(self, data: VideoData):
        """Validate input data for delta encoding."""
    ...
    def process(self, data: VideoData) -> VideoData:
        """
        Encode video frames using delta encoding with optional RLE compression.
        
        Args:
            data: Input VideoData
            
        Returns:
            VideoData with delta-encoded frames and updated metadata
        """
        ...
        LogManager.log_info(
            self.name,
            f"Starting delta encoding: {data.frame_count} frames, "
        ...
        try:
            ...
            LogManager.log_info(
                self.name,
                f"Delta encoding complete: {processing_time:.2f}s processing time"
            )
            
            return processed_data
            
        except Exception as e:
            LogManager.log_error(self.name, f"Delta encoding failed: {str(e)}", e)
            raise ProcessingError(f"Delta encoding failed: {str(e)}", component=self.name)
        ...
        try:
            ...
            if self.use_gpu and CUDA_AVAILABLE:
                with cp.cuda.Device(self.gpu_device):
                    ...
                    # Compute deltas
                    deltas = compute_frame_deltas_gpu(frames_gpu)
                    
                    # Optional RLE compression
                    if self.use_rle:
                        deltas = compress_deltas_rle_gpu(deltas)
                    
                    # Transfer back to CPU
                    result_frames = cp.asnumpy(deltas)
            else:
                result_frames = compute_frame_deltas_cpu(data.frames, workers=self.workers)
                if self.use_rle:
                    result_frames = compress_deltas_rle_cpu(result_frames, workers=self.workers)
    ...
    def _process_in_batches(self, data: VideoData, batch_size: int) -> VideoData:
        ...
        progress = ProgressReporter(frame_count, "Delta encoding frames")
